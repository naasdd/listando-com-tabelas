...
const consultarEntrega = async (req,res)=>{
    ...
    console.log(valor)
    try{
        const pesq = await Entrega.findOne({where: { local: valor.local}})
        if(pesq === null){
            console.log('Local não encontrada no banco de dados!')
            res.status(404).json({message: "Local não encontrada no banco de dados!"})
        }else{
            console.log(pesq)
            res.status(200).json(pesq)
        }
...
module.exports = { cadastrarEntrega, listarEntrega, 
